...
public class ProbabilityCalculator
{
    ...
    public double[][] calculate(Boolean hit, int x, int y, shipSunk typeSunk)
    {
        ...
        calculateProbabilities(cellProbabilityArray, statusArray, size);
        return cellProbabilityArray;
    }

    // Calculate the probability for each cell and update probability array
    private void calculateProbabilities(double [][] cellProbabilityArray, cellStatus [][] statusArray, int size)
    {
        for (int x = 0; x < 10; x++) // columns
        {
            for(y = 0; y < 10; y++) // rows
            {
                if(statusArray[x][y] == cellStatus.blank)
                {
        ...
        switch(typeSunk)
        {
            ...
            case carrier:
            {
                    ...
                    //If only one possible position, update status of those cells to sunk
                {
                    if(x - 4 >= 0)
                    {
                        if(statusArray[x - 4][y] == cellStatus.hit && statusArray[x - 3][y] == cellStatus.hit && statusArray[x - 2][y] == cellStatus.hit && statusArray[x - 1][y] == cellStatus.hit)
                        {
                            left = true; // could be to the left
                            possiblePositions++;
                        }
                    ...
                    {
                        if(statusArray[x + 1][y] == cellStatus.hit && statusArray[x - 1][y] == cellStatus.hit && statusArray[x - 2][y] == cellStatus.hit && statusArray[x - 3][y] == cellStatus.hit)
                        {
                            middleLeft = true; // could be in the middle left
                            possiblePositions++;
                        }
                    ...
                    {
                        if(statusArray[x - 2][y] == cellStatus.hit && statusArray[x - 1][y] == cellStatus.hit && statusArray[x + 1][y] == cellStatus.hit && statusArray[x + 2][y] == cellStatus.hit)
                        {
                            middleHorizontal = true; // could be in the middle
                            possiblePositions++;
                        }
                    ...
                    {
                        if(statusArray[x][y - 4] == cellStatus.hit && statusArray[x][y - 3] == cellStatus.hit && statusArray[x][y - 2] == cellStatus.hit && statusArray[x][y - 1] == cellStatus.hit)
                        {
                            up = true; // could be up
                            possiblePositions++;
                        }
                    ...
                    {
                        if(statusArray[x][y-1] == cellStatus.hit && statusArray[x][y+1] == cellStatus.hit && statusArray[x][y+2] == cellStatus.hit && statusArray[x][y+3] == cellStatus.hit)
                        {
                            middleDown = true; // could be in the middle down
                            possiblePositions++;
                        }
                    ...
                    {
                        if(statusArray[x][y+2] == cellStatus.hit && statusArray[x][y+1] == cellStatus.hit && statusArray[x][y-1] == cellStatus.hit && statusArray[x][y-2] == cellStatus.hit)
                        {
                    ...
                    if(possiblePositions == 1) // update status to sunk
                    {
                        ...
                        if(left == true)
                        {
                            ...
                            statusArray[x - 2][y] = cellStatus.sunk;
                            statusArray[x - 3][y] = cellStatus.sunk;
                            statusArray[x - 4][y] = cellStatus.sunk;
                        }
                        else if(right == true)
                        {
                            statusArray[x + 1][y] = cellStatus.sunk;
                        ...
                        else if(up == true)
                        {
                            ...
                            statusArray[x][y - 2] = cellStatus.sunk;
                            statusArray[x][y - 3] = cellStatus.sunk;
                            statusArray[x][y - 4] = cellStatus.sunk;
                        }
                        else if(down == true)
                        {
                            statusArray[x][y + 1] = cellStatus.sunk;
